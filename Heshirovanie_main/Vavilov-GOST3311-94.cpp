...
// Îôèöèàëüíàÿ òàáëèöà çàìåí (S-áëîê) äëÿ ÃÎÑÒ 34.11-94
...
// Ôóíêöèÿ øèôðîâàíèÿ îäíîãî áëîêà
...
{
    // Èíèöèàëèçàöèÿ íà÷àëüíîãî âåêòîðà
    uint32_t h[2] = { 0, 0 };

    // Äîïîëíåíèå ñîîáùåíèÿ íóëÿìè
    ...
    for (size_t i = 0; i < paddedSize; i += 8) {
        ...
        // Îáíîâëåíèå âåêòîðà õåøà
        gostBlockEncrypt(block, reinterpret_cast<char*>(h));
    }

    delete[] paddedData;

    // Ïðåîáðàçîâàíèå õåøà â ñòðîêó
    std::string result;
    for (int i = 0; i < 2; ++i) {
        char buffer[9];
